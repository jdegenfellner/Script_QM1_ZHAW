...
There are of course events that have not happened ever before, like the creation of artificial general intelligence ([AGI](https://en.wikipedia.org/wiki/Artificial_general_intelligence)). 
...
Especially one of the early eminent statisticians, [Ronald Fisher](https://en.wikipedia.org/wiki/Ronald_Fisher), was an opponent of Bayesian statistics, 
...
More generally, in Frequentist statistics, we are looking for **a fixed but unknown parameter from** an underlying data generating process (DGP). 
...
Note that even if the probability of event A changes when B has happened, B could still have no *causal* effect on A. They could have a common cause, for instance.
...
$$\Omega = \{ (R1pos, R2pos), (R1pos, R2neg), (R1neg, R2pos), (R1neg, R2neg) \}$$ 
in a [Venn diagram](https://en.wikipedia.org/wiki/Venn_diagram). The probabilities of these
...
#### Discrete probability distributions are used when we can count the outcomes, which includes infinitely many. {#discrete_probability_distributions}
...
where $X$ is the [random variable]((https://www.youtube.com/watch?v=Y9nsL69CwbU&t=18s&ab_channel=VeryNormal)) (which takes values $x_i$ when the random experiment is conducted) and $x_i$ are the possible outcomes of $X$.
...
Above in [Addition of probabilities](#addition_of_probabilities) we went through in detail the case of 2 researchers finding an effect. 
...
Let's consider again the 2 researcher situation from above ([Addition of probabilities](#addition_of_probabilities)).
...
```{r, echo=FALSE, fig.width=5, fig.height=3, out.width="65%", fig.align="center"}
library(ggplot2)

...
#### Discrete probability distrubtions are used when we can count the outcomes, which includes infinitely many.
...
```{r}
plot(dbinom(0:20, size = 20, prob = 1/6))
```
...
This is true for all discrete probability distributions. Rather simple and elegant.
...
$X \in \mathbb{Z}$. Every whole number gets the following probability:
$\mathbb{P}(X=0) = 0.1$ and if $X \neq 0$: $\mathbb{P}(X) = 0.2^{|X|}$. 
The sum of all probabilities is: $\sum_{i \in \mathbb{Z}} \mathbb{P}(X=i) = \mathbb{P}(X=0) + 2 \cdot \sum_{i \in \mathbb{N}} 0.2^i = 
...
$\mathbb{P}(X=0) = \frac{1}{6}$ and if $X \neq 0$: $\mathbb{P}(X) = \frac{5}{3} 0.2^{|X|}$. 
...
plot(X_values, probabilities, type = "h", lwd = 2, col = "blue",
     ...
     main = "Probability Distribution of X from -10 to 10")
...
### Prominent probability distributions used in health sciences {#prominent_probability_distributions_in_health_sciences}

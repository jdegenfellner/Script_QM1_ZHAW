...
ggplot(data.frame(value = data), aes(x = value)) +
  geom_histogram(binwidth = 1, color = "blue",
                 fill = "blue", linewidth = 0.1) +  # Narrow blue lines
  geom_vline(xintercept = 1000 * 0.04) +
  labs(title = "Histogram of 1000 1000-researcher experiments",
...
**The fifth question** asked about the problem of [multiple testing](https://en.wikipedia.org/wiki/Multiple_comparisons_problem) and related to the previous question. 
...
So, with just the parameters we have instant information where the values are and where new values are likely to be.

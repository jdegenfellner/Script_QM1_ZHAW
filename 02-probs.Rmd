...
talking about their experiments or their research methodology to each other. The statistician in the department has calculated 
...
We now conduct an experiment. All 1000 ressearches are conducting a study with 50 patients to answer the same question. This is our random experiment. 
...
3. How many experiments showed a treatment effect if you assume that the therapy is working for 12% (instead of 4%) of the patients?
...
5. Assuming 1 very motivated researcher has tested 65 (secondary) hypotheses in her experiments and found 4 results that are difficult to explain by chance alone. What is the problem?

We will try answer such questions in the following sections.
...
Again, this refers to being mutually exclusive within our 1000-researcher experiment. Both events cannot happen at the same time in this context.

An example of **non-disjoint** events (within our 1000-researcher experiment) would be the event $A_1= $"researcher 45 gets a positive result" and the event $A_3= $"researcher 45 and reasearcher 67 gets a positive result".
...
add up the probabilities here, since we would count the probability of researcher 45 twice.

The probability of the event "" should be zero, were "" denotes the event that no researcher gets a positive or negative result ( = $\emptyset$, the so-called empty set). This is impossible due to the design of the experiment.
...
\begin{align}
1. &\ \mathbb{P}(\emptyset) = 0 \text{: Probability of the "impossible" event.}\\
...
The $\infty$-symbol in **axiom 3**. comes into play if we are dealing with (potentially) infinitely many events. For instance, we could ask for the probability of the
treatment effect being between $0$ and $0.1$, or between $0.11$ and $0.2$, and so on. We could construct infinitely many such (disjoint) intervals and law 3 should still hold.
...
Still small, but much higher than before.

As we will see later, **axiom 1** above does not mean, that the event can never occur. For every conintuous random variable, the probability of a single point is zero.

**Axiom 2** is always true. Some result *has* to occur in our random experiment. 
But **what is $\Omega$**? In our countable case of researchers, $\Omega = \{ \omega_1, \omega_2, \cdots, \omega_{2^{1000}} \}$ would be the set of all possible outcomes if we let 1000 researchers conduct the experiment. 
...
We can calculate the probability of observing 137 or more positive results using the binomial distribution (plug into the formula): 

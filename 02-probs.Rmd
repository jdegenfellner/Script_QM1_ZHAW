...
Both events cannot happen at the same time in this context so we assign $0$ to the event that both occur simultaneously. The $\cap$-Symbol refers to all elements 
that are in both sets. In our case we have the sets $A_1 = \{ (\dots ,R_{45} = pos, \dots ,R_{897} = neg, \dots) \}$ and $A_2 = \{ (\dots ,R_{45} = neg, \dots , R_{897} = pos, \dots) \}$.

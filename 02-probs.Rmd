...
Of course, this is not the methods fault but the fault of the user.
...
Computation time is often still an issue in comparison for instance in regression modelling when using a end user laptop.
...
#### Example in applied health sciences (physiotherapy)
...
Now, you run a small trial with 50 patients and observe that 20 of them showed a clinically relevant improvement in mobility (self-reported from the patient). 
This new data (the result of the trial) updates your belief about the effectiveness of the therapy.
Using Bayes' theorem, you combine the prior probability \( p(\theta) = 0.30 \) with the likelihood of the new data \( p(\text{data} | \theta) \), and you calculate 
...
We need to know some basic concepts of probability theory in order dive in deeper. 

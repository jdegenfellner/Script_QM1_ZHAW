...
The event $A_1=$"researcher 45 gets a positive result" and $A_2=$"researcher 897 gets a positive result" are mutually exclusive. We would ask here:
Which researchers got a positive results in both events? The answer is: None, zero, empty set. Hence, the two events are said to disjoint.
...
We can summarize these informally stated properties more formally:
...
The $\infty$-symbol in 3. comes into play if we are dealing with (potentially) infinitely many events. For instance, we could ask for the probability of the
...
In our example, the probability that exacatly 3 researchers find an effect is $\binom{1000}{3} \cdot 0.04^3 \cdot 0.96^{997} = 2.244627 \cdot 10^{-14}$. 
Still small, but much higher than before.


As we will see later, law 1 above does not mean, that the event can never occur. For every conintuous random variable, the probability of a single point is zero.

Law 2 is always true. Some result *has* to occur in our random experiment.

...
observing 137 or more positive results: $1 - \mathbb{P}(\text{observing 136 or less})$. In general, for an event $A$: 
...
In, general, we can write the probability of the union of two events as:
...
We can also visualize the 4 disjoint events ($\Omega = \{ (R1pos, R2pos), (R1pos, R2neg), (R1neg, R2pos), (R1neg, R2neg) \}$) in a [Venn diagram](https://en.wikipedia.org/wiki/Venn_diagram). The probabilites of these
4 events in the event space must add up to 1 since they are disjoint and one of them has to happen. There is no "room" left.

```{r echo=FALSE}
# Create a plot for the event space without borders and with the square aligned on the axes
plot(c(-1, 1), c(-1, 1), type = "n", xlab = "", ylab = "", xaxt = 'n', yaxt = 'n', bty = "n", asp = 1)
...
As one of the most prominent examples of a discrete distribution, we have already seen the [binomial distribution](https://en.wikipedia.org/wiki/Binomial_distribution) in our 1000-researcher-experiment.
...
Discrete or continuous, the **goal** is the same: We want to now **where** the realization of my **random variable** lands **with what probability** 
...
we throw the dice 20 times and are interested in the number of 3:
...
Deviations from zero ($\pm 1$) are highly likely with this distribution. The probability of $X=0$ is also rather high with 0.1.
...
#### Continuous probability distrubtions are used when we cannot count the outcomes. {#continuous_probability_distributions}
The most famous continuous probability distribution is the [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution).
```{r}
...
ggplot(df, aes(x = x, y = density)) +
  ...
  geom_ribbon(data = df_shaded, aes(ymin = 0, ymax = density), fill = "blue", alpha = 0.3) +  # Shaded area
  ggtitle(paste("Standard Normal Distribution: N(", mu, ", ", sigma^2, ")", sep = "")) +
  xlab("X") +
  ylab("Density") +
  theme_minimal() +
  geom_vline(xintercept = mu, color = "red", linetype = "dashed") +
  annotate("text", x = mu + 0.2, y = max(y_vals)/2, label = paste("E(X) =", mu), color = "red") +
  geom_vline(xintercept = a, color = "black", linetype = "dashed") +
  geom_vline(xintercept = b, color = "black", linetype = "dashed") +
  annotate("text", x = a - 0.2, y = max(y_vals)/4, label = paste("a =", a), color = "black") +
  annotate("text", x = b + 0.2, y = max(y_vals)/4, label = paste("b =", b), color = "black") + 
  theme(plot.title = element_text(hjust = 0.5))
```

Here, like in any other "nice" continuous distribution, the area under the curve is 1: $\int_{-\infty}^{\infty} f(x) dx = 1$.
...
See [exercise 1](#exercise1_descriptive_stats) in the next chapter for a practical example for what we will use this in research.

**Expectation $\mathbb{E}(X)$ of a continuous random variable**: The expectation of a continuous random variable $X$ is defined as:
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nec nulla auctor, tincidunt nunc nec, ultricies nunc.
...
### Exercise 2 - Bayes-teaser {#exercise2}
...
### Exercise 3 - Find journals {#exercise3}
...
### Exercise 6 - Three researchers {#exercise6}
...
### Exercise 7 - Conditional probability {#exercise7}
...
### Exercise 8 - Invent a discrete probability distribution {#exercise8}
...
### Exercise 9 - Continuous probability distributions {#exercise9}
...
Hint: You can use simple shapes for the densitiy function defined by lines.

### Exercise 10 - MSc-ZHAW-distribution {#exercise10}
 ...
 - Create sufficiently many random numbers (sample) from the MSc-ZHAW-distribution (see [above](#discrete_probability_distributions)) 
   and see if you can produce values outside of $\pm 10$.

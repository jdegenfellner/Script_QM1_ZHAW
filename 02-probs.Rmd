...
\begin{align}
\tiny
$\mathbb{P}(\text{"(only) researchers 34, 56 and 777 get a pos. result" or "(only) researchers 1 and 5 get a pos. results"}) =\\
\mathbb{P}(\text{"(only) researchers 34, 56 and 777 get a pos. result"}) + \mathbb{P}(\text{"(only) researchers 1 and 5 get a pos. results"})$
\end{align}

As we will see later, law 1 does not mean, that the event can never occur. For every conintuous random variable, the probability of a single point is zero.

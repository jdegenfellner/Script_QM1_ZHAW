...
- **[$p$-hacking](https://de.wikipedia.org/wiki/P-Hacking)**: In the pursuit for "significant" results 
  ...
  that probabilities are defined *before* and event has happened. The probability for the esteemed reader to win the
  Swiss lottery is rather small and I would take a large bet against it. But every week, someone wins (approximately).
  After the fact, one should not be surprised *that* someone one, 
...
The upper limit of $1$ occurs since we have a one-sided test. 95% is just convention and has no special meaning.
...
(see 4.2. [here](https://errorstatistics.com/wp-content/uploads/2015/07/cox-1977-with-discussion-and-reply-nc.pdf)).
...
We could again verify the test statistic by hand if we wanted, the $p$-value is calculated quickly:
```{r}
...
The last assumption listed is that $\bar{X} - \mu$ and $s$ are independent.
At least for large $n$, this should hold. Because of the CLT $\bar{X}$ is 
asymptotically normal
with expectation $\mu$ and variance $\frac{\sigma^2}{n}$. 
...
In the scatter plot, we can clearly see linear relationship between the two variables.
...
There is a simple elegance to this method. It is a very general method and can be applied to many different problems.
...
hist(cors)
quantile(cors, c(0.02, 0.98)) # 96% confidence interval
```

So we use the variability contained in the data to estimate the variability of 
the correlation coefficient. As we can see in the histrogram, correlations close to 0 are
very unlikely. The 96% bootstrap confidence interval is $[-0.93, -0.74]$.

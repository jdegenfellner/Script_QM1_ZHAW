...
df %>% mutate(pre_post_diff = Post_Weight - Pre_Weight) %>%
...
```{r}
mean_diff <- mean(df$Post_Weight - df$Pre_Weight)
sd_diff <- sd(df$Post_Weight - df$Pre_Weight)
n <- nrow(df)
...
Conclusion: It is unlikely that we would see a mean difference of 6.5 kg or more by chance alone 
assuming that the true mean difference is 5 kg or less. We reject the null hypothesis and accept the 
alternative hypothesis that the mean weight loss was more than 5 kg.

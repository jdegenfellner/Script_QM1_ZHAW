...
\end{itemize}
These events are \textbf{independent} because the occurrence of $A$ does not change the probability of $B$ and vice versa. However, they are \textbf{not disjoint} because the numbers 4 and 6 are both even and greater than 3 (so $A \cap B \neq \emptyset$).\\
\\
...
\]
However, they are \textbf{not disjoint} because the number 6 is both even and a multiple of 3 (so $E \cap F \neq \emptyset$).
\end{document}
...
\begin{enumerate}
    ...
    $0 < 0.04*0.96^2 \cdot 0.96^2 0.04 = \\
    \mathbb{P}(\text{only researcher 1 finds an effect}) \times \mathbb{P}(\text{only researcher 3 finds an effect}) \neq \mathbb{P}(\text{only researcher 1 finds an effect}|\text{only researcher 3 finds an effect}) = 0$ since this is impossible, if only researcher 3 found an effect.

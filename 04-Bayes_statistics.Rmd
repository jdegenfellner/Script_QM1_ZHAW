...
[constructed **repeatedly** every time a new sample is drawn](https://en.wikipedia.org/wiki/Confidence_interval#/media/File:Normal_distribution_50%25_CI_illustration.svg). 
...
```{r testing, fig.cap="Bayesian updating: the posterior converges towards the true value as more coin tosses are observed.", echo=FALSE}
library(magick)
img <- magick::image_read("./images/bayesian_updating_sequential.gif")
if (knitr::is_html_output()) {
  # Animated GIF works in HTML output
  img
} else {
  # PDF/LaTeX cannot embed animated GIFs: show the last frame (converged posterior)
  frame <- img[length(img)]
  out <- knitr::fig_path(".png")
  magick::image_write(frame, path = out, format = "png")
...
Let's revisit [Example 1](#example_defective_products) and change the prior probabilities to

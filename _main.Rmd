...
- (Free, purchase of the book is recommended; page numbers in our script refer to the printed version of the book) [Statistical Rethinking](https://github.com/Booleans/statistical-rethinking/blob/master/Statistical%20Rethinking%202nd%20Edition.pdf)
, YouTube-Playlist: [Statistical Rethinking 2023](https://youtu.be/FdnMWdICdRs?si=q2py5QVY_L299hEa)
- (Free with a bit of searching) [Understanding Regression Analysis: A Conditional Distribution Approach](https://vdoc.pub/documents/understanding-regression-analysis-a-conditional-distribution-approach-84oqjr8sqva0)
...
Currently I use a combination of [Github Copilot](https://github.com/features/copilot) (paid), 
[GPT5](https://chatgpt.com/) (paid), and [RStudio](https://posit.co/download/rstudio-desktop/) for writing code and this script. 
...
#### (Made up) Example in applied health sciences (physiotherapy) {#example1_physio}
...
1. If you had to bet, how many experiments showed a treatment effect if you assume that the therapy is not 
working at all? This would be a *prediction*.
...
experiment of 1000 researchers doing their thing *once*. Hence, the two events are said to be disjoint.
...
**How many elementary events are in the set of all possible outcomes of our 1000-researcher experiment?** 
...
### Independence of events {#independence_of_events}

Independence is a very important concept. Whenever you read in a paper that some variables are 
"associated", they mean that they are not statistically independent. It does **not** necessarily 
mean that one variable **causes** the other one to change. It just means that knowing the value 
...
Note, this form of distribution looks like a bell curve, aka [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution), 
...
One of them is the [central limit theorem](https://en.wikipedia.org/wiki/Central_limit_theorem). 
Feel free to watch this [video](https://www.youtube.com/watch?v=zeJD6dqJ5lo&t=28s&ab_channel=3Blue1Brown). 
...
Discrete or continuous, the **goal** is the same: We want to know **where** the realization 
...
The first 2 are absolutely essential.
 ...
 - The most important one is, as mentioned above, the [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution). 
   ...
   Normal distribution theory is very aesthetic and one is sometimes led to believe that this is the *normal* state of nature, which is
   ...
   We will deal with this in [QM2](https://jdegenfellner.github.io/Script_QM2_ZHAW/) in great detail.
...
### [E] Exercise 3 - Find journals and papers {#exercise3}
**Note: This is among the most important exercises of the course:**
Use Google (Scholar) or your favourite search engine to find scientific journals in *your* field (physiotherapy, 
...
**The standardized difference of the group means is $t$-distributed (under the assumption that there is no difference between the groups)**. This case is a bit more complex than the simple ones, 
...
- Are there any **outliers** (rare data points that are far away from the rest, [Westfall, p. 405](https://www.routledge.com/Understanding-Regression-Analysis-A-Conditional-Distribution-Approach/Westfall-Arias/p/book/9780367493516)) and why?
...
more about the population respectively that data generating process (DGP) producing the data [Westfall, p. 6-8](https://www.routledge.com/Understanding-Regression-Analysis-A-Conditional-Distribution-Approach/Westfall-Arias/p/book/9780367493516). 
How did the data come about?
Note that the **sample varies everytime we draw from the population**. 
...
To build a bridge to our concept of statistical [independence](#independence_of_events) from the previous chapter, the paper states: 
"This study found that baseline level of activity limitation was **associated** with different effects of CFT treatment".
...
## Univariate vs. bivariate statisics 
...
p2 <- ggplot(df, aes(x = values)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30, alpha = 0.7, color = "darkgrey") +
  geom_density(aes(y = after_stat(density)), color = "blue", linewidth = 1) +
  geom_boxplot(aes(y = -0.025, x = values), width = 0.05,
               position = position_nudge(y = -0.00)) +
  geom_point(aes(y = -0.025),
             position = position_jitter(width = 0.005, height = 0.025),
             size = 1, alpha = 0.6) +
  ggtitle("Histogram with density plot and boxplot below") +
  theme(plot.title = element_text(hjust = 0.5))
p2
...
**Thirdly**, the sample mean and standard deviation
...
p2 <- ggplot(df, aes(x = values)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30, alpha = 0.7, color = "darkgrey") +
  geom_density(aes(y = after_stat(density)), color = "blue", linewidth = 1) +
  geom_boxplot(aes(y = -0.05, x = values), width = 0.1,
               position = position_nudge(y = -0.00)) +
...
The **sample mean is not robust against outliers**. 
...
Let's assume *Sex* is binomially distributed with a probability for *female* of $p = 0.59$ for both groups.
...
visit [Kruschke's paper](https://www.krigolsonteaching.com/uploads/4/3/8/4/43848243/kruschke2012jepg.pdf), Figure 2.
...
estimated (see Figure 2 [here](https://www.krigolsonteaching.com/uploads/4/3/8/4/43848243/kruschke2012jepg.pdf)). 
...
The animation in [this article](https://mbjoseph.github.io/posts/2018-12-25-animating-the-metropolis-algorithm/) 
...
NHST in his [book](https://nyu-cdsc.github.io/learningr/assets/kruschke_bayesian_in_R.pdf) and
 [article](https://www.krigolsonteaching.com/uploads/4/3/8/4/43848243/kruschke2012jepg.pdf). It seems that for many practinioners, $p$-values are
...
**Are the observed results likely to occur by chance alone if the null hypothesis is true (and other model assumptions are not strongly violated)?**
...
- **[Publication bias](https://en.wikipedia.org/wiki/Publication_bias)**: Many journals tended to publish studies with "significant" results. "Not significant" results
  ...
  conducted studies should be published, irrespective of the results. Estimates of parameters will also be **overestimated**
...
See [Kruschke](https://www.krigolsonteaching.com/uploads/4/3/8/4/43848243/kruschke2012jepg.pdf) for more limitations of NHST.
